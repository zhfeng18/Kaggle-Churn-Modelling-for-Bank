...
```{r glmulti}
# All-subset logistic glmulti
glmulti.logistic.out <-
  glmulti(y ~ ., data = churn.for.best.logistic,
...
# Two-way ANOVA
names(churn)
summary(aov(Exited ~ Gender*age_group, data = churn))
```


```{r Best logistic}
churn.best.logistic <-
    bestglm(Xy = churn.for.best.logistic,
            family = binomial,
            IC = "AIC",
            method = "exhaustive")
churn.best.logistic$BestModels
```

```{r Best model summary}
summary(churn.best.logistic$BestModel)
```

The features in our logistic model are:\
Geography, Gender, Tenure, IsActiveMember, age_group, credit_group, balance_group, product_group
...
churn$product_group <- cut(churn$NumOfProducts, breaks = c(0, 1, 2, 5),
                          labels=c('1', '2', '3_and_over'))
churn1 <- fastDummies::dummy_cols(churn)

churn_clean <- subset(churn1, select = -c(HasCrCard, CreditScore, Geography, Gender, Age, Balance, NumOfProducts, EstimatedSalary, age_group, credit_group, balance_group, product_group))
names(churn_clean)
```
...
```{r Train test split}
set.seed(123)
inds = sample(1:nrow(churn_clean), 0.80 * nrow(churn_clean))
train_churn <- churn_clean[inds,]
test_churn <- churn_clean[-inds,]
```


```{r Logistic regression}
glm1 <- glm(Exited ~ ., data = train_churn, family = binomial)
summary(glm1)
```
...
preds1 <- predict(glm1, newdata = test_churn, type = 'response')
preds1_encode <- ifelse(preds1 >=  0.5, 1, 0)
sum(preds1_encode == test_churn$Exited)
sum(preds1_encode == test_churn$Exited) / nrow(test_churn)
table(Predicted=preds1_encode, Actual=test_churn$Exited) #confusion matrix
```


```{r Logistic regression}
glm2 <- glm(Exited ~ .
            + IsActiveMember*age_group_mid
            + IsActiveMember*age_group_young
            + IsActiveMember*age_group_college
            + IsActiveMember*product_group_1
            + Geography_France*age_group_mid
            + Geography_France*balance_group_Low
            + Geography_France*balance_group_High
            + Geography_Spain*product_group_1
            + age_group_mid*balance_group_None
            + age_group_mid*product_group_1
            + age_group_mid*product_group_3_and_over
            + age_group_young*balance_group_None
            + age_group_young*product_group_3_and_over
            , data = train_churn, family = binomial)
preds2 <- predict(glm2, newdata = test_churn, type = 'response')
preds2_encode <- ifelse(preds2 >=  0.5, 1, 0)
sum(preds2_encode == test_churn$Exited)
sum(preds2_encode == test_churn$Exited) / nrow(test_churn)
table(Predicted=preds2_encode, Actual=test_churn$Exited) #confusion matrix
```

```{r}
inter_all <- glm(Exited ~ (.)^2, data = train_churn)
summary(inter_all)
```
...
```{r}
inter1 <- glm(Exited ~ Geography_Spain*product_group_1, data = train_churn)
summary(inter1)
plot_model(inter1, type = "int")
```
